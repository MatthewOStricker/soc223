...
The coefficients are different between models because the units of one is in miles per hour for one predictor and in kilometers per hour for the other. The intercepts are equal, which means they have equal origins. Also, the slope coefficients are numerically different due to the different units selected, but can be set equal to each other if the units are converted to like terms.
