...
fl_COVID <- read_csv("https://raw.githubusercontent.com/MatthewOStricker/soc223/main/Use%20this%20for%20FLCV20")
...
```{r}
ggplot(pred_data, aes(x = log_cases, y = log_voters))+
  geom_point() +
  geom_line(aes(x = log_cases, y = .fitted), col = "orange") +
  scale_y_continuous(limits = c(0, 10))
```

```{r}
ggplot(pred_data, aes(x = log_cases, y = .resid))+
  geom_point() +
...
```{r}
fl <- state_cases_reduced |>
  filter(State == "FL")

ggplot(COVID_FL, aes(x = InPerson, y = TotalNewCases))+
  geom_point() +
  geom_smooth(method = lm, se = FALSE)
